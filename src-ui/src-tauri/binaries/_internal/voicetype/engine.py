...
class VoiceTypingEngine:
    ...
    def __init__(
        ...
        self._sherpa_keywords = sherpa_keywords or ["小明同学", "你好语音"]
    ...
    async def _on_mic_audio(self, pcm_data: bytes):
        ...
        # === 本地 ASR (Sherpa)：先 VAD + 声纹验证，通过后再发送 ===
        # 计算音频能量，稳定VAD判断
        energy = self._calculate_audio_energy(pcm_data)
        is_speech_frame = energy > 0.015
        
        # 连续3帧有语音才认为是说话开始（60ms快速响应）
        # 连续30帧静音才认为是说话结束（600ms长容错，避免被说话停顿截断）
        if is_speech_frame:
        ...
        elif self._is_speaking and self._silence_frame_count >= 30:
            # 说话结束（连续30帧静音，600ms容错）
            self._is_speaking = False
            self._voiceprint_check_buffer.clear()
        ...
        if self._voiceprint_enabled and self._voiceprint_service and self._is_speaking:
                        ...
                        if vp_files:
                            ...
                            if speaker_id:
                                ...
                                if result.decision:
                                    ...
                                    logger.warning(f"Voiceprint REJECT (score={result.score:.1f} < threshold={self._voiceprint_service.threshold}), blocking ALL audio in this segment")
                                    self._current_speech_verified = False
                                    await self._broadcast({
            ...
            # 🔥 关键修复：如果当前段话未通过声纹验证，则丢弃所有后续音频帧
            if not self._current_speech_verified:
                logger.debug("Voiceprint not verified yet or rejected, dropping audio frame")
                return  # 🛑 完全阻止音频发送到 ASR
