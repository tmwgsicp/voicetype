            ...
            try:
                async for msg in self._ws:
                    ...
                    if event_type == "session.created":
                        ...
                        stash_text = data.get("text", "") or data.get("stash", "")
                        if stash_text and stash_text.strip():
                            await self.send("partial_text", stash_text.strip())
                            logger.debug("[%s] ASR partial: %s", self.config.name, stash_text.strip())
